...
describe('type()', () => {
    ...
    it('parses header (upper case charset)', () => {
        const type = Content.type('application/json; CHARSET=UTF-8');
        expect(type.mime).to.equal('application/json');
        expect(type.charset).to.equal('utf-8');
    });
...
describe('disposition()', () => {
    ...
    it('parses header (filename with special characters)', () => {
        const header = 'form-data; name="file"; filename="fi@le#%.jpg"';
        expect(Content.disposition(header)).to.equal({ name: 'file', filename: 'fi@le#%.jpg' });
    });
});
...
describe('contentLength()', () => {
    ...
    it('parses valid Content-Length header', () => {
        const length = Content.contentLength('Content-Length: 1234');
        expect(length).to.equal(1234);
    });

    it('throws error on missing Content-Length header', () => {
        expect(() => Content.contentLength()).to.throw('Missing Content-Length header');
    });

    it('throws error on invalid Content-Length format', () => {
        expect(() => Content.contentLength('Content-Length: abc')).to.throw('Invalid Content-Length header format');
    });
});